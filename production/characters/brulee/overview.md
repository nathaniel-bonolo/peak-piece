---
type: character
status: draft
canon: true
aliases:
  - Charlotte Brulee
  - Brulee
tags:
  - character
  - charlotte
  - mirror-world
---

# Charlotte Brulee

## Mirror-Mirror Fruit

Brulee can enter and move through the Mirror World without Gems.

The Fruit itself is hers.

A Gem does not turn the ability on.

## Natural limits

Normal use is constrained by:

- Brulee's stamina
- concentration
- attuned mirror infrastructure
- range / operational horizon
- the number and stability of active crossings

## Military-scale use

Soul Feast pushes the Fruit far beyond ordinary personal transport.

Moving thousands of soldiers, equipment, supplies, and repeated crossings requires [[production/systems/gems|Gem]] support because Brulee's own energy budget is not enough to sustain that scale.

Gems extend output.

They do not remove the need for mirrors, attunement, range, or Brulee herself.

## Related

- [[story/sagas/soul-feast/arcs/soul-feast/arc|Soul Feast]]
- [[production/systems/gems|Gems]]
- [[production/lore/factions/big-mom-pirates|Charlotte Empire]]

## Canon audit

See [[production/characters/brulee/canon-audit|Canon Audit]].

## Enemy adaptation

A Fruit this strategically useful should change enemy behavior.

Kaido knows Brulee exists and has had years to prepare Wano.

Peak Wano therefore restricts ordinary mirrors and convenient reflective infrastructure.

This does **not** mean Brulee can use any shiny surface automatically.

It means Kaido sees no upside in leaving thousands of potential candidates across his home territory.

The audience can encounter the restriction first as a stupid Orochi law.

See [[production/lore/locations/wano|Wano]].
